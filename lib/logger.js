...
function Logger(key, options) {
    ...
    checkStringParam(options.tags, 'Tags', true);
    ...
    if (options.tags) {
        ...
        if (Array.isArray(options.tags)) {
            options.tags = options.tags.filter((tag) => tag !== '').map((tag) => tag.trim()).
                join(',');
        } else throw new Error('Tags should be passed as a String or an Array');
    }

    if (options.timeout) {
        if (!isInt(options.timeout)) throw new Error('Timeout must be an Integer');
