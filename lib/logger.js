...
Logger.prototype.log = function(statement, opts) {
    ...
    if (this._meta_keys > 0) {
        message.meta = this._meta;
    }
    ...
    if (opts) {
        if (typeof opts === 'string') {
            ...
            if (opts.timestamp && isValidTimestamp(opts.timestamp)) { message.timestamp = opts.timestamp; }
            if (opts.context && !opts.meta) { opts.meta = opts.context; }
            if (typeof opts.meta === 'object') {
                ...
                } else { message.meta = stringify(opts.meta); }
            }
        }
    } else if (!this._index_meta && message.meta) { message.meta = stringify(message.meta); }
