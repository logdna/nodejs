...
function Logger(key, options) {
    ...
    if (options.timeout) {
        ...
        if (options.timeout > 1000000) {
            throw new Error('Timeout cannot be longer than 1000000');
        }
    }
...
Logger.prototype.checkParam = function(param, name, optional) {
    ...
    if (!param || !_.isString(param)) {
        ...
        throw new Error(name + ' cannot be longer than 32 chars');
    }
};

Logger.prototype.log = function(statement, opts) {
    if (!statement || !_.isString(statement)) {
        debug('Log function must be called with a line, passed as a String');
        return;
    }
    var message = {
        timestamp: Date.now(),
    ...
    if (opts) {
        if (_.isString(opts)) {
            if (opts.length > 32) {
                debug('Level had more than 32 chars, was truncated');
                opts = opts.substring(0, 32);
            }
            message.level = opts;
        } else {
            this.checkParam(opts.level, 'Level', false);
            this.checkParam(opts.app, 'App', false);
