...
Logger.prototype.log = function(statement, opts) {
    ...
    if (typeof statement === 'string' && this._ambient_keys > 0) {
        statement = JSON.stringify(Object.assign({
            message: statement
        }, this._ambient));
    }
...
Logger.prototype.addProperty = function(key, value) {
    this._ambient[key] = value;
    this._ambient_keys += 1;
};

Logger.prototype.removeProperty = function(key) {
    if (this._ambient[key]){
        delete this._ambient[key];
        this._ambient_keys -= 1;
    }
};
