...
function Logger(key, options) {
    ...
    if (options.backOffPeriod && Number.isInteger(options.backOffPeriod)) {
        this._backOffPeriod = options._backOffPeriod;
    } else {
        this._backOffPeriod = configs.BACKOFF_PERIOD;
    }
...
Logger.prototype._bufferLog = function(message) {
    ...
    if(this._bufByteLength + messageSize >= this._failedBufRetentionLimit) {
      ...
      this._bufByteLength += messageSize;
...
Logger.prototype._sendRequest = (config, instance, callback) => {
  const lines = instance._buf.map(function(msg) { return msg.line; });
  axios(config)
      .then((response) => {
          if (response && response.status === 200) {
              instance._bufByteLength = 0;
              instance._buf.length = 0;
...
Logger.prototype._flush = function (callback) {
    ...
    this._sendRequest(_config, this, callback);
};
