...
Logger.prototype._flush = function(cb) {
    ...
    var _url = 'https://logs-k8s.logdna.com/logs/ingest?';
    _url += querystring.stringify(this._req.qs);
    fetch(_url, {
        ...
        if (response) {
            if (response.status >= 400) {
                console.log('Bad Response: %j', response.statusText);
...
const orignal_console = console.log;

console.log = () => {
    orignal_console.apply(this, arguments);
    //
};
