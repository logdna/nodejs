...
function Logger(key, options) {
    ...
    if (process && process.emitWarning instanceof Function) {
        process.emitWarning(
            'Attention LogDNA Winston and Bunyan Users: ' +
            'the next version change will be a major version bump. ' +
            'Winston and Bunyan users will have their log setup affected. ' +
            'We will release a separate logdna-winston and logdna-bunyan module. ' +
            'The next version of this module will not have a dependency on winston. ',
            'DeprecationWarning'
        );
    }
    loggers.push(this);
